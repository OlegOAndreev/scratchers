    ...
    let app = App::new("xtask")
        ...
        .subcommand(SubCommand::with_name("android")
            ...
            .arg(Arg::with_name("api-level")
                ...
                .default_value("26")
                .required(true))
            .arg_from_usage("--sdk-root 'Path to Android SDK (if not set, search in standard locations)'")
            .arg_from_usage("--ndk-root 'Path to Android NDK (if not set, search in standard locations)'")
