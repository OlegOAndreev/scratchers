...
func HorizontalSumNibbles(buf []byte, from int, to int) uint64 {
...
func HorizontalSumNibblesUnsafe(buf []byte, from int, to int) uint64 {
	var ret uint64
	if to-from < 8 {
		for i := from; i < to; i++ {
			ret += uint64(buf[i])
