...
// v2_loop8_unrolled is a version of v2_loop8 which unrolls the internal loop.

[[stage(compute), workgroup_size({{wg_x}}, {{wg_y}})]]
fn main_v2_loop8_unrolled(
...
{% endif %}
        ...
        for (; i < K - 7u; i = i + 8u) {
            ...
            for (var k = 0u; k < 4u; k = k + 1u) {
                ret[k][0] = ret[k][0] + src_mat.data[(row_idx + k) * K + i] * src_vec[0];
                ret[k][1] = ret[k][1] + src_mat.data[(row_idx + k) * K + i + 1u] * src_vec[1];
                ret[k][2] = ret[k][2] + src_mat.data[(row_idx + k) * K + i + 2u] * src_vec[2];
                ret[k][3] = ret[k][3] + src_mat.data[(row_idx + k) * K + i + 3u] * src_vec[3];
                ret[k][4] = ret[k][4] + src_mat.data[(row_idx + k) * K + i + 4u] * src_vec[4];
                ret[k][5] = ret[k][5] + src_mat.data[(row_idx + k) * K + i + 5u] * src_vec[5];
                ret[k][6] = ret[k][6] + src_mat.data[(row_idx + k) * K + i + 6u] * src_vec[6];
                ret[k][7] = ret[k][7] + src_mat.data[(row_idx + k) * K + i + 7u] * src_vec[7];
            }
        }
